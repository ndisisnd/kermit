...
4. If the resolved `auto_commit` is `true` (see Gate resolution), skip the question and proceed as `yes`. Otherwise use `AskUserQuestion` — question: `(4) Run git commit on your behalf?`, options: `yes`, `no`. On no: emit `Tip: if you commit manually later, run \`/kermit --changelog-sync\` to update the changelog.` and terminate.
