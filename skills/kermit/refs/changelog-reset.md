...
- any `## History` backfill block — treat it as a special case: ask once via
  `AskUserQuestion` (`Fold the History section into dated entries too?`, options `Leave verbatim`,
  `Fold it in`); since it is machine-generated one-liners, default to leaving it verbatim at the
  bottom of the file;
...
3. Set `last_number` in `.claude/kermit/state.json` to the max `N` (preserve all other keys; create the
