...
- `refs/protocol-commit.md`: write each new entry under today's `## <date>` section (opening one if the day is new)
- `refs/changelog-reset.md`: normalise an existing changelog into the dated/grouped layout
- `refs/protocol-release.md`: scan `### [N]` entries when collecting unreleased changes
- `SKILL.md`: update the `--changelog-reset` description to the grouped format
- `CHANGELOG.md`: reformat every entry into dated groups
- `.claude/skills/log-it/SKILL.md`: write grouped-by-date entries to match kermit
- `.claude/skills/log-it/refs/changelog-protocol.md`: sync the format doc (kept byte-identical)
- `RELEASES.md`: add the v2.0.0 user-facing release notes for the previous release
